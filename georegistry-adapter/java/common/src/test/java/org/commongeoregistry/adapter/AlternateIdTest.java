...
public class AlternateIdTest
{
  ...
  public void testSerialize()
  {
    ...
    Assert.assertEquals("ABC-123", jo.get("authority").getAsString());
    Assert.assertEquals("DHIS2", jo.get("authorityLabel").getAsString());

    id = (ExternalId) ExternalId.fromJSON(jo);
    Assert.assertEquals("01", id.getId());
    Assert.assertEquals("ABC-123", id.getAuthority());
    Assert.assertEquals("DHIS2", id.getAuthorityLabel());
  }
