...
public class LocalizedValue
{
  ...
  /**
   * Returns a map where the key is the (locale.toString()) and the value is the
   * localized value for that locale.
   */
  ...
  /**
   * Populates the locales in this object with a map where the key is
   * (locale.toString()) and the value is the localized value for that locale.
   * We will NOT set the 'localizedValue' as part of this. If you want to do
  ...
  /**
   * This equals behaviour is required for proper operation of ticket #568. This
   * method is invoked in VertexServerGeoObject.areValuesEqual. You can run
   * RegistryVersionTest to make sure this functionality still works after
  ...
  public boolean equals(Object obj)
  {
    if (obj instanceof LocalizedValue)
    {
      ...
      {
        if (entry.getKey() != null && this.localeValues.get(entry.getKey()) != null && !this.localeValues.get(entry.getKey()).equals(entry.getValue()))
        {
          return false;
        }
      }
  ...
  public static LocalizedValue fromJSON(JsonObject jObject)
  {
    ...
    for (int i = 0; i < locales.size(); i++)
    {
      ...
      if (locale.has(VALUE) && locale.has(LOCALE)) // See ticket #293 if you
                                                   // wonder how it may not have
                                                   // a value
      {
