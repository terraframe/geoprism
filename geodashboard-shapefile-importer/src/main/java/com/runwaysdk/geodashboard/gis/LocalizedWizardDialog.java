 ...
 * 
 * Contributors: IBM Corporation - initial API and implementation Chris Gross
 * (schtoo@schtoo.com) - patch for bug 16179 Eugene Ostroukhov
 * <eugeneo@symbian.org> - Bug 287887 [Wizards] [api] Cancel button has two
 * distinct roles Paul Adams <padams@ittvis.com> - Bug 202534 - [Dialogs] SWT
 * error in Wizard dialog when help is displayed and "Finish" is pressed
 *******************************************************************************/
 ...
 * </p>
 */
public class LocalizedWizardDialog extends TitleAreaDialog implements IWizardContainer2, IPageChangeProvider
{
  /**
   ...
   */
  public LocalizedWizardDialog(Shell parentShell, IWizard newWizard)
  {
    super(parentShell);
      ...
      if (timeWhenLastJobFinished == -1)
      {
        ...
        getShell().addTraverseListener(new TraverseListener()
        {
          public void keyTraversed(TraverseEvent e)
          {
            ...
            {
              // We want to ignore the keystroke when we detect that it has been
              // received within the
              // delay period after the last operation has finished. This
              // prevents the user from accidentally
              // hitting "Enter" or "Space", intending to cancel an operation,
              // but having it processed exactly
              // when the operation finished, thus traversing the wizard. If
              // there is another operation still
              // running, the UI is locked anyway so we are not in this code.
   ...
   * 
   * @see
   * org.eclipse.jface.dialogs.Dialog#setButtonLayoutData(org.eclipse.swt.widgets
   * .Button)
   */
  ...
  private Button createCancelButton(Composite parent)
  {
    ...
    button.setText(Localizer.getMessage("CANCEL"));
   ...
   * method calls call <code>IWizard.addPages</code> to allow the current wizard
   * to add extra pages, then <code>super.createContents</code> to create the
   * controls. It then calls <code>IWizard.createPageControls</code> to allow
   * the wizard to pre-create their page controls prior to opening, so that the
   * wizard opens to the correct size. And finally it shows the first page.
   */
  ...
  protected ProgressMonitorPart createProgressMonitorPart(Composite composite, GridLayout pmlayout)
  {
    ...
    return new ProgressMonitorPart(composite, pmlayout)
    {
       ...
       * 
       * @see
       * org.eclipse.jface.wizard.ProgressMonitorPart#beginTask(java.lang.String
       * , int)
       */
       ...
       * 
       * @see
       * org.eclipse.jface.wizard.ProgressMonitorPart#setTaskName(java.lang.
       * String)
       */
       ...
       * 
       * @see
       * org.eclipse.jface.wizard.ProgressMonitorPart#subTask(java.lang.String)
       */
   ...
   * progress, this method posts an alert message saying that the wizard cannot
   * be closed.
   * 
