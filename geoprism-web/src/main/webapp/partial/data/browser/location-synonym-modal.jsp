      ...
      <form class="modal-form" name="ctrl.form">    
        <div class="modal-dialog">
          <div class="modal-content" show-on-ready>
            <div class="heading">
              <h1 ng-show="entity.id"><gdb:localize key="location.management.synonymModalHeader"/></h1>
            </div>
            <fieldset>
                ...
                <tbody>
                  <tr ng-repeat="synonym in synonyms" class="fade-ngRepeat-item" ng-cloak>
                    <td class="button-column">                       
                      <a class="fa fa-trash-o ico-remove" ng-click="ctrl.removeSynonym(synonym)" title="<gdb:localize key="location.management.removeSynonymTooltip"/>"></a>                       
                    </td>
                    <td class="label-column">
                      <input type="text" ng-model="synonym.displayLabel" name="label" required="required" placeholder="<gdb:localize key="location.management.synonymLabelPlaceholder"/>">
                    </td>
                  </tr>
                  <tr>
                    <td class="button-column">
                      <a class="fa fa-plus" ng-click="ctrl.newSynonym()" title="<gdb:localize key="location.management.createSynonymTooltip"/>"></a>
                    </td>
                    <td ng-show="data.synonyms.size > 0" class="submit-form">
                      <form name="ctrl.form">
                        <input class="list-table-input" type="text" ng-model="synonym.displayLabel" press-esc="ctrl.cancel()"  press-enter="ctrl.apply()" focus-on-show></input>                    
                      </form>
                    </td>
                  </tr>
                </tbody>
