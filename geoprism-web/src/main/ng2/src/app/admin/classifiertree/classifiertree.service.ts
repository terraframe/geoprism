...
@Injectable()
export class ClassifierTreeService extends BasicService {
  
  constructor(service: EventService, private ehttp: EventHttpService, private http: Http) {
    super(service); 
  }
  
  getInstance(): Promise<ClassifierTree> {
    return this.ehttp
      .get(acp + '/tree/classifier')
      .toPromise()
      .then(response => {
        return response.json() as ClassifierTree;
      })
