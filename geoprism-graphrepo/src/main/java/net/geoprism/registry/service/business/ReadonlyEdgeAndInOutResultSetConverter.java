...
public class ReadonlyEdgeAndInOutResultSetConverter extends ResultSetConverter
{
  ...
  public static List<EdgeAndGOInOut> convertResults(List<ReadonlyEdgeAndVerticies> results, Date date, ClassificationBusinessServiceIF cService, ClassificationTypeBusinessServiceIF cTypeService)
  {
    ...
    for (ReadonlyEdgeAndVerticies result : results)
    {
      ...
      {
        GeoObject out = currentOutVertex.toGeoObject(currentOutAttributes, date, cService, cTypeService);
        GeoObject in = currentInVertex.toGeoObject(currentInAttributes, date, cService, cTypeService);
