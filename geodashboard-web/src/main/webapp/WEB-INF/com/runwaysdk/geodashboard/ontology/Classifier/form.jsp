  ...
  <section class="form-container">
    <mjl:component param="dto" item="${item}">
      <div class="field-row clearfix">
        <label for="classifierId">${item.classifierIdMd.displayLabel} <c:if test="${item.classifierIdMd.required}">*</c:if></label>
        <mjl:input id="classifierId" param="classifierId" type="text" />
        <mjl:messages attribute="classifierId" classes="error-message" />
      </div>
      <div class="field-row clearfix">
        <label for="classifierPackage">${item.classifierPackageMd.displayLabel} <c:if test="${item.classifierPackageMd.required}">*</c:if></label>
